...
//	private static final int dimension = 3; 	// 拼图的维数
	...
	public JigsawNode(JigsawNode jNode) {
		nodesState = (int[]) jNode.nodesState.clone();
		nodeDepth = jNode.nodeDepth;
		parent = jNode.parent;
		estimatedValue = jNode.estimatedValue;
	}
	...
	/**获取节点状态，即一个N*N+1的一维数组。第1位代表空白格所处位置
	 * 其余N*N位分别代表每一格中所放方块的数值（按照先行后列排序）。
	...
	public boolean equals(Object obj) {
		for (int i = 0; i < nodesState.length; i++)
			if (nodesState[i] != ((JigsawNode) obj).nodesState[i])
				return false;
		return true;
	}
	...
	public boolean[] canMove() {
		...
		movable[0] = nodesState[0] > dimension && nodesState[0] <= dimension * dimension; // 空白格可向上移
		movable[1] = nodesState[0] >= 1 && nodesState[0] <= dimension * (dimension - 1); // 空白格可向下移
		movable[2] = nodesState[0] % dimension != 1; // 空白格可向左移
		movable[3] = nodesState[0] % dimension != 0; // 空白格可向右移
		return movable;
	}
	...
	public boolean canMoveEmptyUp() {
		return (nodesState[0] > dimension && nodesState[0] <= dimension * dimension);
	}
	...
	public boolean canMoveEmptyDown() {
		return (nodesState[0] >= 1 && nodesState[0] <= dimension * (dimension - 1));
	}
	...
	public boolean canMoveEmptyLeft() {
		return (nodesState[0] % dimension != 1);
	}
	...
	public boolean canMoveEmptyRight() {
		return (nodesState[0] % dimension != 0);
	}
	...
	public boolean moveEmptyUp() {
		int emptyPos = nodesState[0];
		if (nodesState[0] > dimension && nodesState[0] <= dimension * dimension) {
			parent = new JigsawNode(this);
			nodeDepth++;
			
			nodesState[emptyPos] = nodesState[emptyPos - dimension];
			nodesState[emptyPos - dimension] = 0;
			nodesState[0] = emptyPos - dimension;
	...
	public boolean moveEmptyDown() {
		int emptyPos = nodesState[0];
		if (nodesState[0] >= 1 && nodesState[0] <= dimension * (dimension - 1)) {
			parent = new JigsawNode(this);
			nodeDepth++;
	...
	public boolean moveEmptyLeft() {
		int emptyPos = nodesState[0];
		if (nodesState[0] % dimension != 1) {
			parent = new JigsawNode(this);
			nodeDepth++;

			nodesState[emptyPos] = nodesState[emptyPos - 1];
			nodesState[emptyPos - 1] = 0;
			nodesState[0] = emptyPos - 1;
			return true;
		}
		return false;
	}
